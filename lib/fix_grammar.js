...
class SimpleTransformer {
	constructor(lineText, textToCheck) {
		this.lineText = lineText
		this.textToReplace = textToCheck
...
function chooseTextTransformer() {
	...
	return new SimpleTransformer(lineText, textToCheck);
}

async function fixGrammar() {
	const textTransformer = chooseTextTransformer();
	const lineText = textTransformer.lineText;
	...
	try {
		...
		if (error.cause.code === 'ECONNREFUSED') {
			const errorMessage = 'Error connecting to Ollama, make sure it is running.'
...
async function askIfWeShouldReplaceIt() {
	const question = 'Do you want to replace it with the corrected version?';
